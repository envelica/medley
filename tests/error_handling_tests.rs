...
use medley::ebnf::{ParseEvent, grammar, parse_str};
...
fn test_all_alternation_branches_fail() {
    ...
    assert!(
        ...
        "Should not produce token events when all alternation branches fail"
    );
}
...
fn test_extra_input_after_match() {
    ...
    assert_eq!(
        token_events.len(),
        1,
        "String literal emits single token event"
    ); // "test"
}
...
fn test_nested_repetition_error() {
    ...
    assert!(
        has_error,
        "Should error in nested repetition with no matches"
    );
}
...
fn test_character_class_at_eof() {
    ...
    assert!(
        true,
        "Documents behavior when repetition reaches EOF: has_error={}",
        has_error
    );
}
...
fn test_optional_with_error() {
    ...
    assert!(
        true,
        "Documents behavior with optional: has_error={}",
        has_error
    );
}
...
fn test_empty_character_class_match() {
    ...
    assert!(
        true,
        "Documents behavior with zero-or-more on empty input: has_error={}",
        has_error
    );
}
...
fn test_negated_class_match() {
    let g = grammar! {
        // Accept one or more letters; digits should fail
        start ::= letter { letter };
        letter ::= 'a'..'z';
    };
    ...
    assert!(
        has_error,
        "Negated class should fail when input matches excluded range"
    );
}
